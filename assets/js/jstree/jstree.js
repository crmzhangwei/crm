	...
	$.jstree.core.prototype = {
		...
		enable_node : function (obj) {
			...
			this.trigger('enable_node', { 'node' : obj });
                ...
                enable_all: function(){
                    var i, j;
			this._data.core.enabled = this._model.data['#'].children_d.concat();
			for(i = 0, j = this._data.core.enabled.length; i < j; i++) {
				if(this._model.data[this._data.core.enabled[i]]) {
					this._model.data[this._data.core.enabled[i]].state.disabled = false;
				}
			}
			this.redraw(true);
			this.trigger('enable_all');
                },
		/**
